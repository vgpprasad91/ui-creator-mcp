  ...
  tools: [
    {
      ...
      description:
        "Get the component manifest. Without a category, returns a compact index (name, category, plugin for each component). " +
  ...
  switch (name) {
    case "get_component_manifest": {
      ...
      if ((args as any)?.component) {
        const name = (args as any).component;
        const comp = components.find((c: any) => c.name === name || (c.aliases && c.aliases.includes(name)));
        if (comp) {
      ...
      if ((args as any)?.category) {
        ...
        const filtered = components.filter(
          (c: any) => c.category === cat || c.plugin === cat,
        );
      ...
      const index = components.map((c: any) => ({
        name: c.name,
