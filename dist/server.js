...
// ─── KV Client ───────────────────────────────────────────────────
async function kvPut(config, key, value) {
    if (!config.accountId || !config.apiToken || !config.kvNamespaceId) {
        throw new Error("Missing Cloudflare credentials. Set CF_ACCOUNT_ID, CF_API_TOKEN, and CF_KV_NAMESPACE_ID environment variables.");
    }
    ...
    resources: [
        {
            ...
            description: "Complete registry of all 99 components, their props, plugins, and 16 page templates. Read this FIRST before generating any config.",
...
server.setRequestHandler(ListToolsRequestSchema, async () => ({
    tools: [
        ...
        {
            name: "get_templates",
            description: "List all 16 available page templates. Templates are pre-built page configs you can use as-is or customize with overrides. " +
        ...
        },
        {
            ...
            description: "Validate a page config JSON before publishing. Checks structure, required fields, and verifies component types exist in the manifest.",
            inputSchema: {
                type: "object",
                properties: {
                    config: {
        ...
        },
        {
            ...
            description: "Save a page config to the local app directory. Use this to preview configs before publishing. " +
        ...
        },
        {
            ...
            description: "Publish a page config directly to Cloudflare KV, making it live immediately. " +
        ...
        },
        {
            ...
            description: "Publish the app manifest.json to KV. The manifest defines routes, eager/lazy loading, and merge strategies. " +
        ...
        {
            name: "publish_theme",
            description: "Publish a theme config to KV. Controls colors, fonts, radius, and dark mode for the entire app.",
            inputSchema: {
                type: "object",
                properties: {
                    theme: {
        ...
        },
        {
            ...
            description: "Publish datasource definitions to KV. Datasources define how pages fetch data from D1, KV, HTTP APIs, or tRPC.",
            inputSchema: {
                type: "object",
                properties: {
                    datasources: {
        ...
        },
        {
            ...
            description: "Publish navigation config to KV. Defines the sidebar menu items, their icons, paths, and grouping.",
            inputSchema: {
                type: "object",
                properties: {
                    navigation: {
        ...
        {
            name: "add_page_route",
            description: "Add a new route to the app manifest. This fetches the current manifest from KV, adds the route, and re-publishes. " +
        ...
        {
            name: "get_app_status",
            description: "Check the current status of the app — what configs are published, what pages exist, and the runtime URL.",
            inputSchema: {
                type: "object",
                properties: {},
            },
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    ...
    switch (name) {
        ...
        case "validate_page_config": {
            const errors = validatePageConfig(args.config);
            if (errors.length === 0) {
        ...
        case "publish_manifest": {
            ...
            await kvPut(config, key, JSON.stringify(args.manifest));
            return { content: [{ type: "text", text: `Published manifest to KV (key: ${key}).` }] };
        }
        case "publish_theme": {
            const key = `config:${config.appId}:theme`;
            await kvPut(config, key, JSON.stringify(args.theme));
        ...
        case "publish_datasources": {
            ...
            await kvPut(config, key, JSON.stringify(args.datasources));
        ...
        case "publish_navigation": {
            ...
            await kvPut(config, key, JSON.stringify(args.navigation));
            return { content: [{ type: "text", text: `Published navigation config to KV.` }] };
        }
        case "add_page_route": {
            const { path: routePath, file: routeFile, public: isPublic, guard } = args;
            ...
            const filtered = routes.filter((r) => r.path !== routePath);
            const newRoute = { path: routePath, file: routeFile };
            if (isPublic)
                newRoute.public = true;
            if (guard)
                newRoute.guard = guard;
